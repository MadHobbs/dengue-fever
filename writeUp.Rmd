...
2) Simple Environmental-Only Random Forest
    • "current" environmental factors as predictors
    • no time lags
    • year and week not included as predictors
    
3) Environmental Data with Time Lags Random Forest
    • "current" environmental factors as predictors
    • environmental time lags as predictors
    • year and week not included as predictors
...
We look at mean absolute error (MAE), root mean squared error (RMSE), and Normalized RMSE (NRMSE) to compare actual and predicted values. Although MAE will the the measrue used by the DrivenData team to assess our model, NRMSE is perhaps the most useful value to us. NRMSE is root mean squared error as a percentage of the standard deviation of the observations and thus ranges from 0 to 100 (see the [nrmse function](https://www.rdocumentation.org/packages/hydroGOF/versions/0.3-10/topics/nrmse)). Normalizing RMSE in this way helps us compare error across different models or data sets with different scales. We have two data sets with different scales (San Juan experienced many more dengue cases than Iquitos), so NRMSE will be useful to us. 
...
### With current climate varibles only
...
The best predictive models for cases of dengue fever in San Juan, Puerto Rico and Iquitos,Peru were found using current climate data (weather and vegetative) and weather data from previous weeks (lagged data) as explanatory variables in a random forest model. We were able to use these models as the basis of a tool which allows someone to predict cases of dengue fever based on the environment. The environmental conditions are thought to contribute to patterns in dengue fever outbreaks because environmental conditions effect mating and survival of mosquitoes, which are vectors for dengue fever. We have shown that just a few metrics of environmental conditions is sufficient to give a rough indication of the expected out breaks in San Juan, Puerto Rico and Iquitos,Peru. By using different training data it would be possible to develop models for any area of interest. This could be useful in prioritizing which areas receive assistance in possible large scale outbreaks. This model is limited by considering only the past environmental situation. [It is has been proposed](http://www.sciencedirect.com/science/article/pii/S0013935116303127) that in the future  current patterns of outbreaks will be disrupted, as climate change effects regular weather patterns and urbanization disturbs mosquito habitat (standing water where mosquitoes can lay eggs). The burden of dengue fever has been estimated to have increased 30-fold in 50 years, and the increase is expected to continue if there is no intervention.  