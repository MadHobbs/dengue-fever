...
server <- function(input, output, session) {
  ...
  newEntry <- observeEvent(input$addrow, {
    ...
    if(input$which_city == "sj" | input$which_city == "iq") {
      ...
      todayDate <-input$now
      startWeek <- format(as.Date(as.character(todayDate), format="%Y%m%d")-days(4), "%Y%m%d")
      lagDate <- format(as.Date(as.character(todayDate), format="%Y%m%d")-weeks(lag_num_weeks), "%Y%m%d")
      starWeekLag <- format(as.Date(lagDate, format="%Y%m%d")-days(4), "%Y%m%d")
      ...
      startWeek <- format(as.Date(as.character(todayDate), format="%Y%m%d")-days(4), "%Y%m%d")
      lagDate <- format(as.Date(as.character(todayDate), format="%Y%m%d")-weeks(lag_num_weeks), "%Y%m%d")
      startWeekLag <- format(as.Date(lagDate, format="%Y%m%d")-days(4), "%Y%m%d")
